...
import { Text, View, Dimensions, Modal, TouchableHighlight, Animated, ScrollView, Easing, LayoutChangeEvent, Appearance, StyleSheet, Platform, useWindowDimensions, SafeAreaView } from 'react-native'
...
const Button = React.memo(({ buttonTitle, index, darkMode, onPress, ...props }) => {
...
const ActionSheetUI = (props) => {
	const { height: deviceHeight } = useWindowDimensions()

	const maxHeight = Math.round(deviceHeight * 0.8)
			...
			>
				...
				<ScrollView indicatorStyle={darkMode ? "white" : "black"} scrollEnabled={scrollEnabled} style={scrollEnabled ? { flex: 1 } : {}}>
					{props.options.map((title, index) => {
						return props.cancelButtonIndex === index ? null : <Button key={index} buttonTitle={title} index={index} darkMode={darkMode} {...props} />
					})}
				</ScrollView>
			</BlurView>
